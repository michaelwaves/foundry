  ...
  const run = useCallback(async () => {
    ...
    try {
      const motifPayload = form.motif
        ...
        : null
      const isSymmetric = form.mode === 'symmetric'
      const { job_id } = await submitJob(
        {
          design_name: form.designName,
          diffusion_steps: form.diffusionSteps,
          contig: isSymmetric ? null : form.contig.trim() || null,
          length: form.length.trim() || null,
          hotspots: isSymmetric ? {} : parseHotspots(form.hotspots),
          infer_ori_strategy: isSymmetric ? 'none' : form.inferOriStrategy,
  ...
  return (
    <div className="px-8 py-10 max-w-6xl mx-auto">
      <header className="mb-8">
        <h1 className="font-heading text-4xl text-foreground tracking-tight">Generate</h1>
        <p className="text-sm text-muted-foreground mt-1">Configure inputs and run a new RFDiffusion job.</p>
      </header>
