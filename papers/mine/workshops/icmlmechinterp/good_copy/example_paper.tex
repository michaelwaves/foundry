...
\begin{abstract}
All-atom protein diffusion models can be repurposed to design hazardous proteins, but the activations that drive a hazardous denoising trajectory are opaque. We train Matryoshka BatchTopK sparse autoencoders (SAEs) on the diffusion-transformer residual stream of RFdiffusion3 and on the corresponding folding model RoseTTAFold3, using a length-matched corpus of 275 virulent (SafeProtein) and benign (UniProt) sequences. Logistic regression probes on SAE features classify hazardous from benign designs at AUROC $0.817 \pm 0.10$ from RFdiffusion3 block 12 under homology-clustered splits, beating the matched raw-activation probe by $+0.054$ AUROC. Univariate scoring with Benjamini--Hochberg correction recovers individual SAE features that fire on hazardous designs at up to AUROC $0.84$ ($q < 10^{-13}$), and feature quality grows with depth in the diffusion stack. To our knowledge this is the first SAE trained on an all-atom diffusion model and the first feature-level virulence audit of a protein design model.
\end{abstract}

\section{Introduction}
\label{sec:intro}

Generative protein design has reached the point where atom-resolved diffusion models can produce binders, enzymes, and oligomers that fold and bind in the wet lab \cite{watson2023rfdiffusion,krishna2024rfaa,butcher2025rfdiffusion3}. The same models can in principle be conditioned on the active sites of toxins, viral surface proteins, or other hazardous templates. Existing biosecurity screens lean on sequence homology or sequence-level deep classifiers \cite{garg2008virulentpred,singh2024vfpred,sun2024dtvf,chen2025virulenthunter}. They flag a sequence but say nothing about \textit{why} the model is producing it, and they cannot intervene during sampling.

Mechanistic interpretability offers an alternative path: read the residual stream during generation and ask which directions correspond to hazardous structural features. Sparse autoencoders (SAEs) \cite{bricken2023monosemanticity,cunningham2024sparse,templeton2024scaling} have become the workhorse for decomposing language-model activations into approximately monosemantic directions, and recent work has extended them to protein language models \cite{simon2025interplm,adams2025mechanistic,parsan2025interpretable}. Diffusion-based protein design models have not been similarly probed, with the partial exception of FoldSAE on the original RFdiffusion \cite{zarzecki2025foldsae}, which recovered mostly secondary-structure features in a token-level model.

We train SAEs on RFdiffusion3 (RFD3) \cite{butcher2025rfdiffusion3}, an all-atom diffusion model whose internal representations operate on per-residue and per-atom tokens, and on its folding counterpart RoseTTAFold3 (RF3) \cite{krishna2024rfaa}. We then ask whether the resulting feature dictionary supports a meaningful biosecurity readout. Concretely:
...
The best probe trained on RFD3 block-12 SAE features reaches AUROC $0.817 \pm 0.10$ under homology-clustered splits (n=55 held-out designs) and $0.877 \pm 0.025$ under random splits, beating the corresponding raw-activation probe by $+0.054$ AUROC under clustering. Both are below the published bacterial-VF SOTA DTVF \cite{sun2024dtvf} at AUROC $0.92$. Our contribution is not a new classifier; it is a feature-level account of why the diffusion model is acting hazardously, accessible inside the generation loop. We release a suite of SAEs at three depths in RFD3 and two in RF3, plus a small catalogue of significantly hazard-aligned features.
