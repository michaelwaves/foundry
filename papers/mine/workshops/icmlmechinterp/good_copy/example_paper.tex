...
\begin{abstract}
Generative models have shown remarkable progress in a variety of domains such as protein design, but such power enables the opaque generation of hazardous proteins. In this work, we set out to illuminate the black box of RoseTTAFold3 and RFDiffusion3, popular open weight models for protein folding and synthesis, via a mechanistic lens through training Sparse Autoencoders (SAEs) on their diffusion-transformer activations.

We find that for certain blocks, linear probes detect hazardous designs significantly better when fit in the SAE latent space over the original model's representations: improving interpretability without sacrificing model performance.

Furthermore, we identify monosemantic features from the SAE that fire only on hazardous designs at up to AUROC $0.84$ ($q < 10^{-13}$).

To our knowledge this is the first SAE trained on an all-atom diffusion model and the first feature-level virulence audit of a protein design model, paving the way towards safe and interpretable protein design.
\end{abstract}

\section{Introduction}
\label{sec:intro}

Generative protein design has a long history in machine learning \cite{woolfson2021brief} and reached the point where all-atom diffusion models, models operating at the level of individual atoms instead of amino acids, can produce novel protein structures that bind to existing proteins and nucleic acids, significantly speed up chemical reactions, and are experimentally validated to fold in a wet lab. \cite{watson2023rfdiffusion,krishna2024rfaa,butcher2025rfdiffusion3} One well known pernicious feature about protein designing generative models is that the same models can in principle be conditioned on the active sites of toxins, viral surface proteins, or other hazardous templates. Existing biosecurity screens lean on sequence homology or sequence-level deep classifiers \cite{garg2008virulentpred,singh2024vfpred,sun2024dtvf,chen2025virulenthunter}. They flag a sequence but say nothing about \textit{why} the model is producing it, and they cannot intervene during sampling.

Mechanistic interpretability has shown great promise in the understanding of Large Language Models \cite{cunningham2024sparse,adams2025mechanistic,bricken2023monosemanticity} and vision models \cite{joseph2025prisma, simonyan2014deep, Ben_Melech_Stan_2024_CVPR}, and offers an alternative path for understanding in modern protein models: read the intermediate latent features during generation and ask what latent attributes correspond to hazardous structural features. Sparse autoencoders (SAEs) \cite{bricken2023monosemanticity,cunningham2024sparse,templeton2024scaling} have become the workhorse for decomposing language-model activations into approximately monosemantic attributes (that is linear directions in the latent space that only activate on a single semantic concept). And while recent work has extended them to protein language models \cite{simon2025interplm,adams2025mechanistic,parsan2025interpretable}, all-atom diffusion-based protein design models have not been similarly probed. 

We train SAEs on RFdiffusion3 (RFD3) \cite{butcher2025rfdiffusion3}, an all-atom diffusion model whose internal representations operate on per-residue (amino acid) and per-atom tokens \matt{idk what this means} \michael{should we do a brief explainer on protein models and terminology?}, and on its folding counterpart RoseTTAFold3 (RF3) \cite{krishna2024rfaa}. We then ask whether the resulting feature dictionary supports a meaningful biosecurity readout with the following research questions:

\begin{itemize}
    \item \textbf{Do SAE features support virulence classification?} We train logistic regression probes on per-design pooled feature vectors for both RFD3 and RF3.
    \item \textbf{Do SAE features beat raw activations?} We compare under matched probes, splits, and seeds.
    \item \textbf{Are individual features hazard-aligned?} We score every SAE feature against the virulent/benign label with univariate AUROC and Benjamini--Hochberg-corrected $q$-values.
    \item \textbf{Does feature quality depend on depth?} We compare SAEs trained on early, middle, and late blocks of the diffusion stack.
\end{itemize}
\matt{I dislike this whole RQ stack because it's clear we did it post hoc. We didn't have these as questions first and then run experiments. I'd rather just breifly summarize the goals of the work, not have 4 items}

The best probe trained on RFD3 block-12 SAE features reaches AUROC $0.817 \pm 0.10$ \matt{auroc is highly specific to mech int people. I dont really know what its measuring, just precision v.s. recall right? Can we just call it Precision-Recall ROC? People will know that much better.} \michael{so i think AUROC is receiver operating characteristic, which is True positive rate on y and true negative rate on x. AUPRC is different, True positive rate on x and recall on y} under homology-clustered splits (n=55 held-out designs) and $0.877 \pm 0.025$ under random splits, beating the corresponding raw-activation probe by $+0.054$ AUROC under clustering. 
\matt{dont even mention the following stuff yet, I think the contribution of SAE features improve interpretability over regular feautres is strong enough (for a workshop)}
Both are below the published bacterial-VF SOTA DTVF \cite{sun2024dtvf} at AUROC $0.92$. 
Our contribution is not a new classifier; it is a feature-level account of why the diffusion model is acting hazardously, accessible inside the generation loop. 

We will release a suite of SAEs at three depths in RFD3 and two in RF3, plus a small catalogue of significantly hazard-aligned features.
...
\caption{Held-out probe AUROC by (model, block, encoding) under random vs.\ homology-clustered splits. RFD3 block 12 SAE features lead in both regimes; RFD3's earlier blocks lose much more under clustering than RF3, consistent with family memorization in early atomistic representations. \matt{all 3 of these figures could probably just be tables.}} \michael{ye i guess all this info is already in Table 1, but it's easier to highlight certain trends and diffs visually this way? perhaps we just keep figure 2}
...
\caption{Top-$10$ hazard-firing SAE features per (model, block) ranked by univariate AUROC. Each bar is annotated with the BH-corrected $q$-value. RFD3 block 12 carries individual features at AUROC $\geq 0.80$ with $q \leq 10^{-7}$. \matt{do you have a qualitative example(s) of a risky looking protein that fires only on 1 or some of these?}}
...
\caption{Eval AUROC for our best probe alongside published bacterial-VF classifiers. VirulentHunter \cite{chen2025virulenthunter} reports relative AUC gains over MP4/DeepVF but no absolute AUROC and is omitted. \matt{this doesnt need to be a chart. It can be in text and talk about comparison to SOTA, and explain its not a fair comparison because we arent training for this task, just building explanation models}}
\label{fig:sota}
\end{figure}
...
\paragraph{What the SAE adds.} The probe results are not, on their own, surprising: large protein generative models have learned representations from which structural and functional properties can be linearly read \cite{lin2023evolutionaryscale,adams2025mechanistic}. The new piece is that the residual stream of an all-atom diffusion model has the same property, and that a Matryoshka BatchTopK SAE can isolate hazard-aligned directions cleanly enough that a low-capacity probe outperforms its raw counterpart at the right depth. Significance-corrected univariate scoring then localizes that signal to a small set of directions one can inspect and intervene on.
...
We trained sparse autoencoders on the residual stream of an all-atom protein diffusion model, RFdiffusion3, and on its folding counterpart RoseTTAFold3. SAE features support hazardous/benign classification at AUROC up to $0.88$ random and $0.82$ homology-clustered, beating raw activations by $+0.054$ AUROC at the right depth, and individual SAE directions correlate with virulence at AUROC up to $0.84$ with strict FDR control. Block 12 of RFD3 is the depth at which the trade-off between feature disentanglement and class-discriminability is most favourable. The result is a feature dictionary that is competitive with sequence-only classifiers and, unlike them, points at structural reasons the model is producing what it does. We see this as a starting point for runtime monitoring, steering, and audit of generative protein design models.
...
This work is dual-use. The same SAE features that support hazard classification could in principle support hazard generation if used as conditioning signals. We restrict the released catalogue to feature indices and AUROCs and withhold per-residue activation maps for top hazard features; the ranked feature list is enough to reproduce the classifier and the limitation analysis but does not amount to a recipe for a hazardous design. The longer-term motivation, runtime monitoring of generative protein design models, is defensive: a feature-level audit of an in-flight generation is a more fine-grained safeguard than a sequence-level post-hoc screen.
