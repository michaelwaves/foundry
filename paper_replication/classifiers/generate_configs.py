...
def _rel(p: Path) -> str:
    """Return path relative to ROOT so generated configs work from any machine."""
    try:
...
def _emit_score(name: str, dataset: dict, hook: str, hook_spec: dict, extractor: str) -> None:
    config = {
        "activations_path": _rel(dataset["activations"]),
        "labels_path": _rel(dataset["labels_dir"] / "labels.csv"),
        "hook_name": hook,
...
def _emit_fit(name: str, dataset: dict, hook: str, hook_spec: dict,
    ...
    config = {
        "activations_path": _rel(dataset["activations"]),
        "labels_path": _rel(dataset["labels_dir"] / f"labels_fold{fold}_train{_LABELS_SUFFIX}.csv"),
        "hook_name": hook,
        "extractor": _extractor_block(dataset, hook, hook_spec, extractor),
        "pooling": "last_step",
        "aggregation": "mean",
...
def _emit_eval(name: str, dataset: dict, hook: str, extractor: str, fold: int) -> None:
    ...
    config = {
        "bundle_path": _rel(OUTPUT_ROOT / name / "fit" / cell),
        "activations_path": _rel(dataset["activations"]),
        "labels_path": _rel(dataset["labels_dir"] / f"labels_fold{fold}_test{_LABELS_SUFFIX}.csv"),
    }
...
def _extractor_block(dataset: dict, hook: str, hook_spec: dict, extractor: str) -> dict:
    ...
    return {
        "kind": "sae_encode",
        "checkpoint_path": _rel(dataset["sae_root"] / hook / "final.pt"),
        "device": DEVICE,
    }
