...
Replaces the three per-tutorial build_inputs.py scripts. Dispatches on `--model`:
...
def main(
    ...
    run_config = load_or_seed_run_config(out, hooks_yaml)
...
def _build_payload(
    rows: list, model: str, run_config: dict, partial_t: float,
) -> tuple[dict, int, list[str]]:
    if model == "rfd3":
        body, skipped = build_rfd3_payload(rows, partial_t)
